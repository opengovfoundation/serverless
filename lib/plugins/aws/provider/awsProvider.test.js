...
describe('AwsProvider', () => {
  ...
  describe('#getPartition()', () => {
    ...
    it('should return "aws" for "us-east-1"', () => {
      awsProvider.options.region = 'us-east-1';

      expect(awsProvider.getPartition()).to.equal('aws');
    });

    it('should return "aws-us-gov" for "us-gov-west-1"', () => {
      awsProvider.options.region = 'us-gov-west-1';

      expect(awsProvider.getPartition()).to.equal('aws-us-gov');
    });

    it('should return "aws-cn" for "cn-north-1"', () => {
      awsProvider.options.region = 'cn-north-1';

      expect(awsProvider.getPartition()).to.equal('aws-cn');
    });
  });
